<template>
    <div>
        <vs-table max-items="10" search pagination stripe :data="other_pending">
            <template slot="thead">
                ...
                <vs-th sort-key="pending_to_name">Pending To</vs-th>
            </template>
            ...
            axios.get(`/api/dashboard/pending/${this.other}`)
                .then(response => {
                    ...
                    if (response.status === 200) {
                        this.other_pending = response.data.pending;
                    }
                })
                .catch(err => {
