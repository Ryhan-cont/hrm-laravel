...
<div>
    <vx-card >
      <form-wizard color="rgba(var(--vs-primary), 1)" title="Create Company" subtitle="Please compleate all the step" :disabled="btnDisabled" finishButtonText="Update" @on-complete="updateCompany()">
        <tab-content title="Company General Info" icon="feather icon-home">
          ...
          <vs-input label-placeholder="Name" type="text" class="w-full pt-4"  v-model="name" />
          <vs-input label-placeholder="Address" type="text" class="pt-3 w-full" v-model="address" />
        ...
        <tab-content label="Admin" title="Admin" icon="feather icon-user">
          ...
          <div class="py-10">
            ...
            <vs-input label-placeholder="Email" class="pt-3 w-full" name="email" v-model="adminEmail" />

            <vs-input label-placeholder="Phone" class="pt-3 w-full" v-model="adminPhone" />
          </div>
...
export default {
    ...
    methods: {
        ...
        async loadData() {
            ...
            try {
                var req = await axios.post('/api/company-edit/data', data)
                var companyData = req.data.companyData;
                ...
                var adminData = req.data.adminData;
                this.first_name = adminData.first_name;
                this.last_name = adminData.last_name;
                this.adminId = adminData.id;
                this.adminAddress = adminData.address;
                this.adminPhone = adminData.phone;
                ...
                this.packageId = req.data.companyPackage.id;
                this.selectedPackage = req.data.companyPackage.name;
                this.selectedPackageData = req.data.companyPackage;

                this.packageData = req.data.packageData;
                this.packageModule = req.data.packageModuleData;
                this.packageOptions = req.data.packageOptions;

                var companyData = req.data.companyData;
                var CompanyLogo = '/storage/company-' + companyData.id + '/' + companyData.logo;
            ...
            try {
                var req = await axios.get(CompanyLogo);
                this.imgData(CompanyLogo);
            } catch (err) {
                console.log(err)
            }
        ...
        async loadPackage() {
            ...
            try {
                var req = await axios.post('/api/company-edit/data/package/module', data);
        ...
        async updateCompany() {
            ...
            try {
                ...
                var req = await axios.post('/api/company-edit/data/proceed', fornData)
                if (req) {
