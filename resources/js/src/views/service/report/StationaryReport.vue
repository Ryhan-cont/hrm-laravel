<template>
    <div>
        ...
        <vx-card >
            <vs-table max-items="10" pagination search stripe :data="stationaryData">

                <template slot="thead">
                    <vs-th sort-key="">Employee</vs-th>
                    <vs-th sort-key="">Category</vs-th>
                    <vs-th sort-key="">Subcategory</vs-th>
                    <vs-th sort-key="">Item</vs-th>
                    <vs-th sort-key="">Quantity</vs-th>
                    <vs-th sort-key="">Need By Date</vs-th>
                ...
                <template slot-scope="{data}">
                    <vs-tr :key="indextr" v-for="(tr, indextr) in data" >
                        ...
                        <vs-td :data="tr.sub_category"> {{ tr.sub_category }} </vs-td>
                        <vs-td :data="tr.item"> {{ tr.item }} </vs-td>
                        <vs-td :data="tr.quantity"> {{ tr.quantity }} </vs-td>
                        <vs-td :data="tr.need_by_date"> {{ tr.need_by_date }} </vs-td>
        ...
        methods:{
            ...
            fetchReport(target) {
                axios.get("/api/report/stationary-report",{
                    ...
                    .then(response => {
                        this.stationaryData = response.data.stationary_data;
            ...
            exportExcel(){
                ...
                window.open(`/export-stationary?${paramString.toString()}`);
