<template>
    <div>
        ...
        <vx-card >
            <vs-table max-items="10" pagination search stripe :data="loiData">

                <template slot="thead">
                    <vs-th sort-key="">Employee</vs-th>
                    <vs-th sort-key="">Org Type</vs-th>
                    <vs-th sort-key="">Org Name</vs-th>
                    <vs-th sort-key="">Date Range</vs-th>
                    <vs-th sort-key="">Address</vs-th>
                    <vs-th sort-key="">Letter To</vs-th>
                ...
                <template slot-scope="{data}">
                    <vs-tr :key="indextr" v-for="(tr, indextr) in data" >
                        ...
                        <vs-td :data="tr.letter_to"> {{ tr.letter_to }} </vs-td>
    ...
    methods:{
        ...
        fetchReport(target) {
            axios.get("/api/report/loi-report",{
                ...
                .then(response => {
                    this.loiData = response.data.loi_data;
        ...
        exportExcel(){
            ...
            window.open(`/export-loi?${paramString.toString()}`);
