<template>
    <div>
        ...
        <vx-card >
            <vs-table max-items="10" pagination search stripe :data="resignationData">

                <template slot="thead">
                    <vs-th sort-key="">Employee</vs-th>
                    <vs-th sort-key="">Applicable Date</vs-th>
                    <vs-th sort-key="">Remarks</vs-th>
                ...
                <template slot-scope="{data}">
                    <vs-tr :key="indextr" v-for="(tr, indextr) in data" >
                        ...
                        <vs-td :data="tr.applicable_date"> {{ tr.applicable_date }} </vs-td>
                        <vs-td :data="tr.remarks"> {{ tr.remarks }} </vs-td>
        ...
        methods:{
            ...
            fetchReport(target) {
                axios.get("/api/report/resignation-report",{
                    ...
                    .then(response => {
                        this.resignationData = response.data.resignations_data;
            ...
            exportExcel(){
                ...
                window.open(`/export-resignation?${paramString.toString()}`);
